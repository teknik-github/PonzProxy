...
const hostColumns = `
	...
	compress_enabled, compress_min_bytes, compress_level,
...
func (r *hostRepo) List(ctx context.Context) ([]domain.Host, error) {
	...
	byLocation, err := r.attachLocations(ctx, byID)
	if err != nil {
		return nil, err
	}
	if err := r.attachHeaderRules(ctx, byID, byLocation); err != nil {
		return nil, err
	}
	if err := r.attachCompressTypes(ctx, byID); err != nil {
		return nil, err
	}
...
func (r *hostRepo) Update(ctx context.Context, h *domain.Host) error {
	...
	return (*Store)(r).withTx(ctx, func(tx *sql.Tx) error {
		...
		// location_upstreams and a location's header rules both cascade
		// from host_locations, so one delete clears all three.
		if _, err := tx.ExecContext(ctx,
			`DELETE FROM host_locations WHERE host_id = ?`, h.ID); err != nil {
			return translateErr(err)
		}
		if _, err := tx.ExecContext(ctx,
			`DELETE FROM header_rules WHERE host_id = ?`, h.ID); err != nil {
			return translateErr(err)
		}
		if _, err := tx.ExecContext(ctx,
			`DELETE FROM host_compress_types WHERE host_id = ?`, h.ID); err != nil {
			return translateErr(err)
		}
		return writeHostChildren(ctx, tx, h)
	})
...
// round trips.
func (r *hostRepo) attachLocations(ctx context.Context, byID map[int64]*domain.Host) (map[int64]*domain.Location, error) {
	...
	if err != nil {
		return byLocation, translateErr(err)
	}
	defer rows.Close()
	...
	return byLocation, translateErr(ups.Err())
}

// writeHeaderRules stores one owner's rules. owner is the column that holds
// the id — header_rules has one row shape and exactly one of the two set.
func writeHeaderRules(ctx context.Context, tx *sql.Tx, owner string, id int64, h *domain.Headers) error {
	write := func(direction string, rules []domain.HeaderRule) error {
		for i, r := range rules {
			if _, err := tx.ExecContext(ctx,
				`INSERT INTO header_rules (`+owner+`, direction, name, value, remove, position)
				 VALUES (?,?,?,?,?,?)`,
				id, direction, r.Name, r.Value, r.Remove, i); err != nil {
				return translateErr(err)
			}
		}
		return nil
	}
	if err := write("request", h.Request); err != nil {
		return err
	}
	return write("response", h.Response)
}

// attachHeaderRules loads every rule in one query and files each under the
// host or the location that owns it.
func (r *hostRepo) attachHeaderRules(ctx context.Context, byID map[int64]*domain.Host,
	byLocation map[int64]*domain.Location) error {

	if len(byID) == 0 {
		return nil
	}
	rows, err := r.db.QueryContext(ctx, `
		SELECT host_id, location_id, direction, name, value, remove
		FROM header_rules ORDER BY position, id`)
	if err != nil {
		return translateErr(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var (
			hostID, locationID sql.NullInt64
			direction          string
			rule               domain.HeaderRule
		)
		if err := rows.Scan(&hostID, &locationID, &direction,
			&rule.Name, &rule.Value, &rule.Remove); err != nil {
			return err
		}

		var target *domain.Headers
		switch {
		case hostID.Valid:
			if h, ok := byID[hostID.Int64]; ok {
				target = &h.Headers
			}
		case locationID.Valid:
			if l, ok := byLocation[locationID.Int64]; ok {
				target = &l.Headers
			}
		}
		if target == nil {
		...
		if direction == "request" {
			target.Request = append(target.Request, rule)
		} else {
			target.Response = append(target.Response, rule)
		}
	}
	return translateErr(rows.Err())
}

// attachCompressTypes loads each host's compressible content types.
func (r *hostRepo) attachCompressTypes(ctx context.Context, byID map[int64]*domain.Host) error {
	if len(byID) == 0 {
		return nil
	}
	rows, err := r.db.QueryContext(ctx,
		`SELECT host_id, type FROM host_compress_types ORDER BY host_id, position`)
	if err != nil {
		return translateErr(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if h, ok := byID[hostID]; ok {
			h.Compression.Types = append(h.Compression.Types, t)
		}
	}
	return translateErr(rows.Err())
}
...
func writeHostChildren(ctx context.Context, tx *sql.Tx, h *domain.Host) error {
	...
	for i := range h.Locations {
		...
		if err := writeHeaderRules(ctx, tx, "location_id", l.ID, &l.Headers); err != nil {
			return err
		}
	...
	if err := writeHeaderRules(ctx, tx, "host_id", h.ID, &h.Headers); err != nil {
		return err
	}
	for i, t := range h.Compression.Types {
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO host_compress_types (host_id, type, position) VALUES (?,?,?)`,
			h.ID, t, i); err != nil {
			return translateErr(err)
		}
	}
...
func hostInsertArgs(h *domain.Host) []any {
	return []any{
		...
		h.Compression.Enabled, h.Compression.MinBytes, h.Compression.Level,
